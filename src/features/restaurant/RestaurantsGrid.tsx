...
import { restaurantDetails } from "@/lib/constants/restuarant";
          ...
          <Reveal key={r.slug} delay={i * 0.08}>
            ...
            >
              <div
                ...
                style={{ background: `linear-gradient(140deg, ${r.coverFrom}, ${r.coverTo})` }}
              >
                <Icon className="w-16 h-16 text-secondary/70" strokeWidth={1.2} />
                {r.isOpen && (
                  <span className="absolute top-3 left-3 bg-white text-primary-dark text-[11px] font-bold px-3 py-1.5 rounded-full flex items-center gap-1.5">
                    <Circle className="w-2 h-2 fill-primary text-primary" strokeWidth={0} />
                    Open Now
                  </span>
                )}
              </div>
              <div className="px-5 pt-4.5 pb-5">
                ...
                <div className="flex gap-3.5 text-muted-foreground text-[12.5px] font-medium">
                  <span className="flex items-center gap-1">
                    <Clock className="w-3.5 h-3.5" strokeWidth={2} /> {r.deliveryTime}
                  </span>
                  <span className="flex items-center gap-1">
                    <MapPin className="w-3.5 h-3.5" strokeWidth={2} /> {r.distance}
                  </span>
                  <span className="flex items-center gap-1">
                    <Bike className="w-3.5 h-3.5" strokeWidth={2} /> {r.deliveryFee}
                  </span>
                </div>
              </div>
            </Link>
